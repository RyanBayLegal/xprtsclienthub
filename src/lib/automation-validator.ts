  ...
  for (const n of nodes) {
    ...
    switch (kind) {
      case "send_email": {
        ...
        if ((cfg.to_mode ?? "contact") === "contact" && !["lead_created", "lead_created_manual", "lead_merged", "lead_stage_change", "client_stage_change", "email_received", "task_event"].includes(triggerType)) {
