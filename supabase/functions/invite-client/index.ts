  ...
  try {
    ...
    if (resend) {
      ...
      try {
        ...
        await logInvite(adminClient, caller, { email, name }, false, (mailErr as Error).message, "invite_resend");
        return new Response(
      ...
      await logInvite(adminClient, caller, { email, name }, true, null, "invite_resend");
    ...
    await logInvite(adminClient, caller, { email, name, userId }, emailSent, emailError);
